...
void compileFuncDecl(void) {
  ...
  checkFreshIdent(currentToken->string);
  //khoi tao va khai bao, thuong duoc di voi nhau
...
Type* compileLValue(void) {
    //gia tri truoc khai bao, tra ve kieu truoc khai bao
  // TODO: parse a lvalue (a variable, an array element, a parameter, the current function identifier)
  Object* var;
  Type* varType;
  eat(TK_IDENT);
  var = checkDeclaredLValueIdent(currentToken->string); //xem da khai bao chua
  switch(var->kind){
  case OBJ_VARIABLE:
    if (var->varAttrs->type->typeClass==TP_ARRAY)
      varType=compileIndexes(var->varAttrs->type); //neu la Var thi kiem tra Index
    else
...
void compileAssignSt(void) {
  ...
  checkTypeEquality(varType,expType); //so sanh 2 kieu minh moi compile duoc, 2 kieu 2 ben := phai giong nhau
}
...
void compileForSt(void) {
  ...
  Type *expType;
  eat(KW_FOR);
  // eat(TK_IDENT); Biet vi sao bo dong nay ko ? Vi compileLValue no da kiem tra tu truoc do roi
  varType=compileLValue();
...
void compileCondition(void) {
    //dieu kien a = b, a >b ...
    //a va b co the la 2 bieu thuc -->compileExpression truoc, sau do so sanh 2 kieu xem giong nhau hay khong
...
Type* compileExpression(void) {
    //complie bieu thuc, tra ve kieu ma no tra ve
  Type* type;
...
Type* compileExpression2(void) {
  ...
  type1 = compileTerm(); //a*b, a/b, a
  type2 = compileExpression3(type1);
...
Type* compileExpression3(Type *argType1) {
  ...
  case SB_PLUS:
    ...
    type=compileExpression3(argType1);
    break;
  case SB_MINUS:
    eat(SB_MINUS);
    checkIntType(argType1);
